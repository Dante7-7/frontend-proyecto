...
async function submitNewPassword() {
    ...
    try {
        const response = await UsuarioService.guardar_nuevaclave(token, newPassword.value);
        toast.add({
            severity: 'info',
            summary: 'Mensaje',
                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        <div class="reset-password">
                            <h2 class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Restablecer Contraseña</h2>
                            <form @submit.prevent="submitNewPassword">
                                ...
                                <div>
                                    ...
                                    <Password id="password" v-model="confirmNewPassword" placeholder="Confirmar Contraseña" :toggleMask="true" class="w-full md:w-[30rem] mb-8" fluid :feedback="false"></Password>
                                </div>
                                <Button type="submit" class="w-full">Actualizar Contraseña</Button>
                                <p class="mt-4 text-center">
                                    <a @click="redirectTo" class="font-medium ml-2 text-right cursor-pointer text-primary">Salir</a>
                                </p>
                            </form>
                        </div>
                    </div>
